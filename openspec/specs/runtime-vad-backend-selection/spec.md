...
- **THEN** the system SHALL use Whisper's local JSON token timing offsets to suppress words fully covered by the preceding window
- **AND** SHALL retain the timestamps of words that extend into newly captured audio
